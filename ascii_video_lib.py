...
class ascii_video:
    ...
    def display_ascii_video_smooth(self, frames, fps):
        ...
        for i, frame in enumerate(frames):
            ...
            stdout.write("\x1b[H")  # Переместить курсор в верхний левый угол
            stdout.write(frame)
            stdout.flush()
    ...
    def ascii_convert(self, input_path, output, new_width, format):
        ...
        while cap.isOpened():
            ...
            ascii_img, colors = self.frame_to_ascii_color(frame, new_width=new_width)
            if format == 'mp4':
                ascii_frame = self.ascii_to_color_image(ascii_img, colors, font_path, font_size=font_size)
                ascii_frame = cv2.resize(ascii_frame, (width, height))

                out.write(cv2.cvtColor(ascii_frame, cv2.COLOR_RGB2BGR))
            if format == 'html':
                self.save_ascii_html(ascii_img, colors, output_path='frames/frame'+str(counter)+'.html', font_size=font_size)
            if format == 'txt':
                self.save_colored_ascii_to_txt(ascii_img, colors, output_path=output+'video.txt')
